 ...
 * </ul>
 *
 * <p><b>Note on docmind</b> – the expected command from the issue is:
 * <pre>
 * podman run -d --name docmind \
 *   -p 8008:8008 \
 *   -v docmind_pgdata:/var/lib/postgresql/data \
 *   -v docmind_fastembed_cache:/app/.fastembed \
 *   -v $HOME/.docmind:/root/.docmind \
 *   -e PUBLIC_BASE_URL=http://localhost:8008 \
 *   -e FTS_LANG=italian \
 *   -e ENABLE_CHAT=true \
 *   -e DOCMIND_DEFAULT_PROVIDER=copilot \
 *   -e GITHUB_TOKEN=$(gh auth token) \
 *   production.eng.it:8433/intelligence_automation_asset/docmind:latest
 * </pre>
 *
 * <p>The command actually generated by this tool is different: it includes mandatory
 * resource-limiting flags and WSL2 safe defaults, and all values are wrapped in single quotes
 * (preventing shell expansion of {@code $HOME} and {@code $(gh auth token)}).
 * The real command produced by {@code run_container} with docmind parameters is:
 * <pre>
 * podman run -d --name 'docmind' \
 ...
 * </pre>
 *
 * <p>Differences from the expected command:
 * <ul>
 *   <li>{@code --cpus=1}, {@code --memory=512m}, {@code --memory-swap=512m},
 *       {@code --pids-limit=256} are added (mandatory resource limits)</li>
 *   <li>{@code --restart=on-failure:3} and log-size limits are added</li>
 *   <li>{@code --network=slirp4netns} is added (WSL2-safe default)</li>
 *   <li>All values are single-quoted: {@code $HOME} and {@code $(gh auth token)}
 *       are not expanded by the shell</li>
 * </ul>
 */
